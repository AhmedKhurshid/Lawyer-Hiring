...
if (isset($_POST['login'])) {
    ...
    $password = $_POST['password'];
        ...
        $sqli = "SELECT * FROM `users` WHERE `u_email` = '$email' && `u_password` = '$password'" ;
        
        $sql = "SELECT * FROM `lawyer_info` WHERE `l_email`= '$email' && `l_password` = '$password'";

        $sqlii="SELECT * FROM `admin` WHERE `admin_email`='$email' && `admin_password` = '$password'";
            
                $result = mysqli_query($conn, $sql);
                $result2 = mysqli_query($conn, $sqli);
                $result3 = mysqli_query($conn, $sqlii);
                ...
                if($count != 0) {
                    $lid = "SELECT l_id FROM `lawyer_info` WHERE `l_email`= '$email' && `l_password` = '$password'";
                    $re = mysqli_query($conn, $lid);
                    $rowid=mysqli_fetch_row($re);
            
                    $_POST['$email']=='u_email';
              $_POST['$password'] == 'u_password';

              $_SESSION['id']= $rowid[0];
                 header('Location:profile.php?id='.$rowid[0]);
                 
                }
                else if($num != 0){
                    $uid = "SELECT id FROM `users` WHERE `u_email`= '$email' && `u_password` = '$password'";
                    $ure = mysqli_query($conn, $uid);
                    $rowid=mysqli_fetch_row($ure);
                    $_SESSION['id']= $rowid[0]; 
                  $_POST['$email'] == 'l_email';
